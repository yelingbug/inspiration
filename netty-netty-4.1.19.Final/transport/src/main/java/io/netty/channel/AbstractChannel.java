    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                if (isActive()) {//这个表示已经绑定或者已经连接(对NioServerSocketChannel和NioSocketChannel实现不同)．
                    if (firstRegistration) {//如果从来都没有注册，回调一次channelActive，确保多次取消注册再重新注册时只有一次channelActive被调用．
                        pipeline.fireChannelActive();
                    } else if (config().isAutoRead()) {//如果channel之前被注册过并且自动读被设置，再重新注册时要尝试开始读取数据，应该是有一个ｂｕｇ在这里修复了．
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (!wasActive && isActive()) {//绑定前如果是false，绑定后如果是true，说明绑定成功，要回调一次fireChannelActive事件．其实上面516行代码也做了次冗余实现．
