...
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap
    ...
    private void invokeChannelRegistered() {
        if (invokeHandler()) {//要保证这个handle对应的context已经增加到pipeline中或者pending到pipeline中，才能直接调用handle对应的回调方法．
            try {
            ...
            fireChannelRegistered();//如果发现一个handle对应的context还没有增加到pipeline或者pending到pipeline(这种情况发生在调用pipeline增加handle及其对应的context时)，
            //从context链路中找到下一个context，继续判断和调用．
        }
    }
