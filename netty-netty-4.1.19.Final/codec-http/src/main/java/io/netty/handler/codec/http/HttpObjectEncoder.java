                ...
                case ST_CONTENT_NON_CHUNK:
                    ...
                    if (contentLength > 0) {
                        if (buf != null && buf.writableBytes() >= contentLength && msg instanceof HttpContent) {
                            ...
                            buf.writeBytes(((HttpContent) msg).content());//玄机在这里，在HTTP中ctx.writeAndFlush(FullHttpResponse).addListener(ChannelFutureListener.CLOSE)写入的
                            //是HttpContent的子类，这里转换成了字节．
                            out.add(buf);
                        } else {
                            if (buf != null) {
                                out.add(buf);
