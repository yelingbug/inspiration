...
public class Disruptor<T>
{
                                               ...
                                               final EventHandler<? super T>[] eventHandlers)
    {
        ...
        {
            /**
             * 很奇怪，在ConsumerRepository的unMarkEventProcessorsAsEndOfChain方法里对
             * getEventProcessorInfo(barrierEventProcessor)没有判断为空的实现，这是因为
             * 对一个Disruptor实例，ConsumerRepository只有一个，那么在处理EventHandlerGroup#and/then/after
             * 等情况的时候，就实际上隐含在链式处理的前面这些barrierEventProcessor已经保存在ConsumerRepository
             * 里边了。
             * 比如：dw.handleEventsWith(A).then(B);
             * handleEventsWith时，A涉及到的Sequence已经在里边了，那么在then里调用handleEventsWith方法
             * 的时候(会走到这个地方)，就会把已经存在的A涉及到的Sequence对应的ConsumerInfo调用unMarkEventProcessorsAsEndOfChain
             * 方法置属性为false。
             *
             */
