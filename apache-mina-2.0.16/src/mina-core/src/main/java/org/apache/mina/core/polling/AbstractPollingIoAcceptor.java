        ...
        for (;;) {
            ...
            try {
                ...
                if (future.getException() != null) {
                    ...
                    /**
                     * 这里考虑到了先后绑定本地多个端口的情况，<code>registerQueue</code>里会先后存在多个
                     * <code>AcceptorOperationFuture</code>实例.
                     *
                     * 在{@link #registerHandles()} 所在的循环里，如果已经有端口绑定并等待在{@link #select()} 时，
                     * 这里{@link #wakeup()}的目的是唤醒等待的{@link #select()}的继续循环，优先处理绑定，即
                     * {@link #registerQueue}队列里的{@link org.apache.mina.core.service.AbstractIoAcceptor.AcceptorOperationFuture}
                     * 实例．
                     *
                     */
