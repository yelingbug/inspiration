...
public interface RegistryService {
     ...
     */
    void register(URL url);//当注册一个服务提供者时，以SimpleRegistryService为例，增加服务提供者的url到registered列表，同时在subscribed列表中找到订阅了这个url(UrlUtils.isMatch)的listener
    //回调listener(通过subscribe(url,listener)添加)，其中详细的流程可以参照SimpleRegistryService中的注释。
     ...
     */
    void subscribe(URL url, NotifyListener listener);//当订阅一个服务提供者时，以SimpleRegistryService为例，增加服务提供者的url到subscribed列表，同时在registered列表中查找(lookup)所有符合
    //条件的url列表，及时执行回调listener
