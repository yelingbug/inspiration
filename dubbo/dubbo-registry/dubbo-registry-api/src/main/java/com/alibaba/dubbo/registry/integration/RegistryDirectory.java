...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    public synchronized void notify(List<URL> urls) {
        ...
        for (URL url : urls) {
            ...
            String category = url.getParameter(Constants.CATEGORY_KEY, Constants.DEFAULT_CATEGORY);//默认category=provider，这个很重要，因为注册中心的url里缺省情况下是没有category的，有了
            //category=provider，invokerUrls才会有值，refreshInvokers()方法中会更新methodInvokerMap/urlInvokerMap这样重要的变量。
        ...
        if (configuratorUrls != null && !configuratorUrls.isEmpty()) {
            this.configurators = toConfigurators(configuratorUrls);//当
        }
        // routers
    ...
    public List<Invoker<T>> doList(Invocation invocation) {
        ...
        //这里的methodInvokerMap很关键，这个变量是在方法refreshInvoker()方法中更新的，而这个方法是被notify()方法调用，并且在notify()方法中只有对带有参数category=provider的url
        //处理时才有机会更新methodInvokerMap这个变量，逻辑意义上来说，带有category=provider的url一般都是服务提供者在注册中心注册时的url，从代码中看出有几个地方触发了这个方法调用：
        //1. 基于dubbo协议通过DubboRegistryFactory工厂创建Registry接口实现DubboRegistry实例时，对注册中心的url及其BACKUP_KEY的url列表时，DubboRegistry的构造函数中和构建RegistryDirectory
        //    实例时，静态的调用了notify()方法，但这个时候的调用正常来说在注册中心的url中没有category=provider参数。
        //2. AbstractRegistry/AbstractRegistryService(AbstractRegistry->Registry/AbstractRegistryService->RegistryService，具体实现有dubbo/multicast/zookeeper/redis)中在register/unregister/subscribe/unsubscribe
        //    动作时均有触发。
        //在没有服务提供者注册时，这个变量应该是空的。
