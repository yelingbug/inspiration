...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    // registry centers
    protected List<RegistryConfig> registries;//多个注册中心
    ...
    protected void checkRegistry() {
        ...
        for (RegistryConfig registryConfig : registries) {
            appendProperties(registryConfig);//尝试从ＪＶＭ上下文中获取对应的property值注入到RegistryConfig实例中。
        }
    }
    ...
    protected List<URL> loadRegistries(boolean provider) {//provider标识和REGISTER_KEY和SUBSCRIBE_KEY共同决定是注册还是订阅
        checkRegistry();
            ...
            for (RegistryConfig config : registries) {
                ...
                if (address != null && address.length() > 0
                    ...
                    for (URL url : urls) {
                        ...
                        url = url.setProtocol(Constants.REGISTRY_PROTOCOL);//对所有的注册中心ＵＲＬ，使用registry协议，这点至关重要，因为在后面protocol.export要先从注册中心开始
                        if ((provider && url.getParameter(Constants.REGISTER_KEY, true))
                                || (!provider && url.getParameter(Constants.SUBSCRIBE_KEY, true))) {
                            /**
                             * 这里的REGISTER_KEY控制服务提供者是否进行注册，即服务提供者<dubbo:service.../>；SUBSCRIBE_KEY控制服务消费者是否进行订阅，即服务消费者<dubbo:reference.../>；
                             * 对注册来说，一般情况下是RegistryService#register(...)，比如RegistryProtocol#export(...)中的实现；对消费来说，一般情况下是RegistryService#subscribe(...)，比如
                             * RegistryProtocol#refer(...)中的实现。
                             *
                             * 具体参见dubbo.io官网“只注册”/”只订阅“部分的应用场景。
                             */
