...
public class FutureFilter implements Filter {
    ...
    public Result invoke(final Invoker<?> invoker, final Invocation invocation) throws RpcException {
        ...
        fireInvokeCallback(invoker, invocation);//回调oninvoke
        // need to configure if there's return value before the invocation in order to help invoker to judge if it's
        // necessary to return future.
        Result result = invoker.invoke(invocation);
        if (isAsync) {
            asyncCallback(invoker, invocation);//异步，从RpcContext中拿到Future并setCallback，等到有结果返回时调用onthrow/onreturn
        } else {
            syncCallback(invoker, invocation, result);//同步，直接调用onthrow/onreturn
        }
        return result;
    }
