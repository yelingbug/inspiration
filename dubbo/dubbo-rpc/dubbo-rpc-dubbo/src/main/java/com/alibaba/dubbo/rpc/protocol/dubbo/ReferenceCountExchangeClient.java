...
final class ReferenceCountExchangeClient implements ExchangeClient {
    ...
    public void close(int timeout) {//关闭当前ReferenceCountExchangeClient连接的时候(也即关闭持有的ExchangeClient连接)，计数递减，并且判断如果<=0，要彻底的关闭其持有的ExchangeClient
        //，并且用一个延迟连接LazyConnectExchangeClient实例来代替。
        //ghostClientMap的作用在于当用LazyConnectExchangeClient代替的时候要将其放入到ghostClientMap中，在比如DubboProtocol#getSharedClient(URL)初始化ReferenceCountExchangeClient
        //连接的时候会把这个延迟连接给remove掉。
        //理解上来说，就是当关闭ReferenceCountExchangeClient的时候，在计数<=0时，关闭持有的ExchangeClient连接，并且初始化一个LazyConnectExchangeClient代替。
