...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        loadDataBase();
        cnxnFactory.start();        
        startLeaderElection();//启动竞选端口线程,准备接受其他peer的连接,并且对每个连接的peer初始化发送和接受队列.
        super.start();//开始真正的处理选举,自己发起选举投票,接受peer选举投票并进行计算.
    }
    ...
    private void loadDataBase() {
		...
		try {
            ...
            /**
             * 在 ZAB 协议的事务编号 Zxid 设计中，Zxid 是一个 64 位的数字，其中低 32 位是一个简单的单调递增的计数器，针对客户端每一个事务请求，
             * 计数器加 1；而高 32 位则代表 Leader 周期 epoch 的编号，每个当选产生一个新的 Leader 服务器，就会从这个 Leader 服务器上取出其本地
             * 日志中最大事务的ZXID，并从中读取 epoch 值，然后加 1，以此作为新的 epoch，并将低 32 位从 0 开始计数。
             */

            LOG.info("Yelin.G After load database, the lastProcessedZxid = {}", zkDb.getDataTree().lastProcessedZxid);
            // load the epochs
            long lastProcessedZxid = zkDb.getDataTree().lastProcessedZxid;
            /**
             * 因为zxid是由epoch和单调递增的计数器构成,这里尝试从zxid中获取epoch的值.
             */
    		long epochOfZxid = ZxidUtils.getEpochFromZxid(lastProcessedZxid);
            try {
                /**
                 * 如果是集群的第一次启动,这个文件是不存在的,没关系,exception捕捉中会初始化这个文件.
                 *
                 * 如果不是第一次,尝试从文件中回复当前的epoch和从zxid中获取到的epoch比较,这里还要同时判断是否存在
                 * updatingEpoch文件,如果有这个文件存在,并且从zxid中获取的epoch大于当前的,说明在更新currentEpoch之前
                 * 做了snapshot之后服务挂了,更新currentEpoch即可.
                 */
            	...
            	// new code version
            	currentEpoch = epochOfZxid;//如果开始CURRENT_EPOCH_FILENAME文件不存在,用zxid代替,并写入文件.
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                /**
                 * 在每个case发生exception时,都会捕捉并且重新设置当前服务的状态为looking,重新进入选举状态.
                 */
                switch (getPeerState()) {
                case LOOKING:
                    LOG.info("LOOKING");
