...
public class FileTxnSnapLog {
    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, 
        ...
        /**
         * 尝试从排序后的snapshot文件列表中从文件名中获取lastProcessedZxid
         */
        snapLog.deserialize(dt, sessions);//从dataDir目录中取snapshot.xxxxx中后缀序号最大的文件,并把这个序号当做初始的zxid
        /**
         * 基于上面从snapshot中获取到的最后一个zxid,尝试从data目录中排序log文件列表,并读取每个log文件,找到>=(zxid+1)的
         * zxid.
         *
         * 因为snapshot是压缩之后的有效transaction,而log是实时写入的每次请求transaction(还没有压缩),所以系统加载的时候
         * 在这里做了优化,先找到snapshot中的最大一个zxid,再基于这个zxid从log文件中回溯所有的有效transaction到内存树中,
         * 在这个过程中恢复重建整个树.
         */
        return fastForwardFromEdits(dt, sessions, listener);//从dataLogDir中获取log.xxxxx文件名中后缀大于zxid的所有文件,解析日志并做命令回方回复数据
    }
    ...
    public long fastForwardFromEdits(DataTree dt, Map<Long, Integer> sessions,
        ...
        LOG.info("Yelin.G: dt.lastProcessedZxid = " + dt.lastProcessedZxid);
        TxnHeader hdr;
        try {
            while (true) {
                // iterator points to 
