...
final class PoolThreadCache {
    ...
    PoolThreadCache(PoolArena<byte[]> heapArena, PoolArena<ByteBuffer> directArena,
        ...
        /**
         * smallCacheSize默认是{@link PooledByteBufAllocator#DEFAULT_SMALL_CACHE_SIZE} = 256，numSmallSubpagePools=39
         *
         * normalCacheSize默认是{@link PooledByteBufAllocator#DEFAULT_NORMAL_CACHE_SIZE} = 64
         * maxCachedBufferCapacity默认是{@link PooledByteBufAllocator#DEFAULT_MAX_CACHED_BUFFER_CAPACITY} = 32 * 1024(32K)
         *
    ...
    private abstract static class MemoryRegionCache<T> {
        ...
        MemoryRegionCache(int size, SizeClass sizeClass) {
            ...
            /**
             * 利用JCTools的多生产单消费队列结构：之所以使用 Multi-Producer-Single-Consumer 这种队列
             * 是因为：ByteBuf的分配和释放可能在不同的线程中，这里的多生产者即多个不同的释放线程，这样才能
             * 保证多个释放线程同时释放的 ByteBuf 能高效添加到队列中。LinkedTransferQueue
             *
             * 在MPSC的源代码里有很多的继承来做padding，为什么要用继承来做padding，是因为通过long型填充在
             * 一些不同JDK中会被优化掉，源码中用padding层次来隔离producerIndex和consumerIndex保证
             * cache line不会miss。https://www.infoq.com/presentations/jctools-algorithms-optimization/
             * 这篇文章有个比较详细的解释。
             *
             * 为什么要用Mpsc？某个工作线程申请PooledByteBuf资源的时候会找到这个线程的PoolThreadCache副本从
             * 其中的queue.poll()获取(单消费者)，而申请到的PooledByteBuf可能会被其他线程使用最后释放归还，即
             * queue.offer()释放回queue中(多生产者)。
             *
             * commented by Yelin.G on 2021.12.31
             */
