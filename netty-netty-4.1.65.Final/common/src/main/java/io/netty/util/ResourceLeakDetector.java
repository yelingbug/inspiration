...
public class ResourceLeakDetector<T> {
    ...
    private void reportLeak() {
        ...
        for (;;) {
            ...
            if (reportedLeaks.add(records)) {
                if (records.isEmpty()) {
                    /**
                     * untraced和traced的区别在于是否调用了track()类方法，因为进入到refQueue中
                     * 元素都是垃圾回收以后的对象，如果records为空，说明{@link DefaultResourceLeak}中的
                     * 链表为空，可能没有调用track()类方法，但是被回收了。
                     *
                     * commented by Yelin.G on 2021.11.04
                     */
    ...
    private static final class DefaultResourceLeak<T>
         ...
         * properties:(简单来说，要记录的对象可能会很多(缺省是4)，所以每个待记录的对象都有1/2^n概率的机会替换掉最顶层的对象 commented by Yelin.G on 2021.10.18)
         *
         * <ol>
    ...
    private static class TraceRecord extends Throwable {
        ...
        private static final TraceRecord BOTTOM = new TraceRecord() {
            ...
            // 官方解释目的:
            // We have a few classes in which we store and reuse static instances of various exceptions. When
            // doing so it is important to also override fillInStacktrace() and so prevent the leak of the
            // ClassLoader in the internal backtrace field.
            // 简单点的理解就是这个方法会走系统调用回溯调用栈，在这里定义的常量其实是没有必要的。
