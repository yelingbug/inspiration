...
public abstract class Recycler<T> {
    ...
    private static final class DefaultHandle<T> implements Handle<T> {
        ...
        /**
         * compareAndSet和weakCompareAndSet的区别在于weakCompareAndSet保留了volatile在JMM中的语义(happens-before)，
         * 但是不保证处理操作目标的volatile变量外的其他变量的执行顺序。(volatile变量可以保证，比如说在volatile声明前的变量
         * 是可以的)。
         *
         * commented by Yelin.G on 2021.10.30
         */
    ...
    private static final class WeakOrderQueue extends WeakReference<Thread> {
        ...
        void add(DefaultHandle<?> handle) {
            /**
             * 多线程竞争的时候handle只能添加到一个WeakOrderQueue.
             *
             * commented by Yelin.G on 2021.10.30
             */
    ...
    private static final class Stack<T> {
        ...
        private void pushLater(DefaultHandle<?> item, Thread thread) {
            ...
            // 获取当前线程的Map<Stack, WeakOrderQueue>. commented by Yelin.G on 2021.10.31
            Map<Stack<?>, WeakOrderQueue> delayedRecycled = DELAYED_RECYCLED.get();
            /**
             * DELAYED_RECYCLED是{@link FastThreadLocal}，按照线程的维度存储Map<Stack, WeakOrderQueue>副本，
             * 这里的this即Stack可能来自不同的线程(即一个线程里创建对象，另外一个线程里回收)，所以在多线程场景下，每个
             * 线程副本Map<Stack, WeakOrderQueue>都可能会存在对同一个Stack实例的引用，所以在某个线程下初始化Stack
             * 实例对应的WeakOrderQueue时，即newWeakOrderQueue(thread)，会将当前线程下新创建的WeakOrderQueue
             * 挂到Stack实例的WeakOrderQueue链中(因为Stack实例可能在其他线程中已经关联了WeakOrderQueue)，即可能
             * 会出现：
             *
             * Stack.next->WeakOrderQueue(线程1)->WeakOrderQueue(线程2)->WeakOrderQueue(线程3)....
             *
             * 见{@link #newWeakOrderQueue(Thread)} -> WeakOrderQueue.newQueue(this, thread) -> ... ->
             * stack.setHead() -> queue.setNext(head);head = queue;
             *
             * commented by Yelin.G on 2021.10.31
             */
