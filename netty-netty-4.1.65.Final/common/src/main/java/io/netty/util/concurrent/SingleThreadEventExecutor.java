...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    /**
     * {@link io.netty.channel.EventLoop}启动时候关联.
    ...
    /**
     * 启动线程的策略通过{@link SingleThreadEventExecutor#state}变量以及原子更新的方式保证一个{@link EventExecutor}
     * 对应一个线程。
     * 但是线程本质上是被线程池管理的，这个线程池是实现了{@link EventExecutor}接口的{@link ThreadPerTaskExecutor}(在
     * {@link MultithreadEventExecutorGroup}定义的executor, 76行代码).
     * 所以说即使{@link ThreadPerTaskExecutor}没有限制池的大小，但是因为这里基于{@link #state}的状态原子更新，所以一个{@link EventExecutor}
     * 本质上对应一个线程。
     * commented by Yelin.G on 2021.07.06
     */
    ...
    private void doStartThread() {
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
                thread = Thread.currentThread();//分配线程. commented by Yelin.G on 2021.07.28
                if (interrupted) {
                    thread.interrupt();
                }
