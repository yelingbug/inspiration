 ...
 *
 * {@link DefaultAttributeMap}中维护了一个{@link DefaultAttribute}数组，通过{@link #attr(AttributeKey)}获取{@link DefaultAttribute}，
 * 如果不存在，创建一个{@link DefaultAttribute}并按照{@link DefaultAttribute#key()}的顺序去插入，简而言之{@link DefaultAttribute}数组是有序的。
...
public class DefaultAttributeMap implements AttributeMap {
    ...
    private static void orderedCopyOnInsert(DefaultAttribute[] sortedSrc, int srcLength, DefaultAttribute[] copy,
        ...
        // 从后往前走，因为依据经验估计，toInsert.key.id()在新key上更大。
        // commented by Yelin.G on 2021.10.24
    ...
    public <T> Attribute<T> attr(AttributeKey<T> key) {
        ...
        for (;;) {
            ...
            if (index >= 0) {
                ...
                /**
                 * 当{@link AttributeKey}对应的{@link DefaultAttribute}不存在的时候，创建一个
                 * {@link DefaultAttribute}并按照顺序插入。
                 * commented by Yelin.G on 2021.10.08
                 */
                if (newAttribute == null) {
                    newAttribute = new DefaultAttribute<T>(this, key);
    ...
    /**
     * 引用了{@link DefaultAttributeMap}并包含对应的{@link AttributeKey}，继承了{@link AtomicReference}并提供
     * {@link DefaultAttribute}的原子更新和释放操作。
     * commented by Yelin.G on 2021.10.08
     */
