...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    /**
     * 每次写循环最大写消息数量。
     * commented by Yelin.G on 2021.10.15
     */
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public void close(final ChannelPromise promise) {
            ...
            /**
             * 在close的实现中，先调用assertEventLoop方法确保当前方法是在eventLoop中执行，然后多次使用invokeLater方法把一系列操作
             * 放在放在Runnable中执行，这样做的目的是事为了保证接下来的操作一定在当前操作完成之后才会执行，这一点是有eventLoop来保证的，
             * eventLoop执行Runnable的顺序和调用execute的顺序一致，相关实现会在后面eventLoop章节具体讨论。
             * commented by Yelin.G on 2021.10.13
             */
