  ...
  private static final class UnaryServerCallHandler<ReqT, RespT>
    ...
    private final class UnaryServerCallListener extends ServerCall.Listener<ReqT> {
      ...
      public void onMessage(ReqT request) {
        ...
        // half-closes.
        this.request = request;//将解析的消息保存下来,在onHalfClose()的时候才真正的方法调用,那么下面的onHalfClose()是在什么时候调用的呢?
        //实际上是在HTTP/2的请求中碰到流结束标志时,从DefaultHttp2FrameReader的processHeaderState/processPayloadState方法开始通过链式处理追溯到
        //NettyServerHandler中onDataRead()方法,这个方法中有个endOfStream参数如果为true表示流结束,从而调用NettyServerStream.inboundDataReceived()
        //方法,接着调用AbstractStream.closeDeframer()方法,接着调用MessageDeframer.closeWhenComplete()方法,触发MessageDeframer.close()方法,最终触发
        //到这里的onHalfClose()方法,即流结束时开始调用方法
      }
      ...
      public void onHalfClose() {
        ...
        method.invoke(request, responseObserver);//接上面的描述,也即是这里
