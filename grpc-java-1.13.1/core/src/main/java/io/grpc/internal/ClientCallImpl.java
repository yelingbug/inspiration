...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
  ...
  public void start(final Listener<RespT> observer, Metadata headers) {
    ...
    if (!deadlineExceeded) {
      ...
      if (retryEnabled) {//默认情况下是依赖builder.retryEnabled和builder.temporarilyDisableRetry，如果没有明确设置，这个是false
        ...
        ClientTransport transport = clientTransportProvider.get(
            new PickSubchannelArgsImpl(method, headers, callOptions));//第一次走到这里返回的transport是DelayedClientTransport
        Context origContext = context.attach();
        try {
          stream = transport.newStream(method, headers, callOptions);//第一次走到这里返回的是PendingStream
        } finally {
    ...
    stream.start(new ClientStreamListenerImpl(observer));//如果observer没有被interceptor包装的情况下，可能这里的observer=UnaryStreamToFuture，否则可能是在不同的
    //interceptor实现中被wrap成不同的实现。
