    ...
    public void messagesAvailable(final MessageProducer producer) {
      ...
      try {
        while ((message = producer.next()) != null) {
          try {
            listener.onMessage(call.method.parseRequest(message));//这里的listener是UnaryServerCallListener,call中的method是在生成的grpc服务代码中被初始化的,比如下面的例子:
            /**
             * @javax.annotation.Generated(
             *     value = "by gRPC proto compiler (version 1.13.1)",
             *     comments = "Source: bid_service.proto")
             * public final class BidServiceGrpc {
             *
             ...
             *   private static volatile io.grpc.MethodDescriptor<com.hongling.refactoring.exchanger.BiddingInput,
             *       com.hongling.refactoring.exchanger.ResultOutput> getBidMethod;
             *
             *   public static io.grpc.MethodDescriptor<com.hongling.refactoring.exchanger.BiddingInput,
             *       com.hongling.refactoring.exchanger.ResultOutput> getBidMethod() {
             *     io.grpc.MethodDescriptor<com.hongling.refactoring.exchanger.BiddingInput, com.hongling.refactoring.exchanger.ResultOutput> getBidMethod;
             *     if ((getBidMethod = BidServiceGrpc.getBidMethod) == null) {
             *       synchronized (BidServiceGrpc.class) {
             *         if ((getBidMethod = BidServiceGrpc.getBidMethod) == null) {
             *           BidServiceGrpc.getBidMethod = getBidMethod =
             *               io.grpc.MethodDescriptor.<com.hongling.refactoring.exchanger.BiddingInput, com.hongling.refactoring.exchanger.ResultOutput>newBuilder()
             *               .setType(io.grpc.MethodDescriptor.MethodType.UNARY)
             *               .setFullMethodName(generateFullMethodName(
             *                   "BidService", "bid"))
             *               .setSampledToLocalTracing(true)
             *               .setRequestMarshaller(io.grpc.protobuf.ProtoUtils.marshaller(
             *                   com.hongling.refactoring.exchanger.BiddingInput.getDefaultInstance()))
             *               .setResponseMarshaller(io.grpc.protobuf.ProtoUtils.marshaller(
             *                   com.hongling.refactoring.exchanger.ResultOutput.getDefaultInstance()))
             *                   .setSchemaDescriptor(new BidServiceMethodDescriptorSupplier("bid"))
             *                   .build();//看这里看这里看这里看这里看这里看这里
             *           }
             *         }
             *      }
             *      return getBidMethod;
