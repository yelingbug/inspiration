    ...
    try {
      synchronized (lock) {
        ...
        if (state.getState() == IDLE) {
          gotoNonErrorState(CONNECTING);//状态变更时要辗转到最后调用LB的handleSubchannelState()方法去处理，比如在涉及到IDLE->CONNECTING等
          startNewTransport();
        }
      }
    } finally {
  ...
  private void startNewTransport() {
    ...
    ConnectionClientTransport transport =
        new CallTracingTransport(
            transportFactory.newClientTransport(address, authority, userAgent, proxy),//这里的transportFactory默认情况下就是NettyChannelBuilder.NettyTransportFactory
            callsTracer);
      ...
      try {
        synchronized (lock) {
          ...
          if (savedShutdownReason != null) {
            ...
            gotoNonErrorState(READY);//transport已经OK切换到READY状态，追溯下去会走到DelayedTransport.reprocess()方法，而这个方法会针对pendingStreams待处理的流集合
            //进行处理，比如说在第一次进入时候走到DelayedClientTransport.createPendingStream()时给pendingStreams中增加的流；
            //在reprocess()方法中对pendingStream流集合进行处理时，会调用PendingStream.createRealStream()方法，这个方法会基于已经准备好的transport创建真正的网络流，默认
            //就是NettyClientStream，并且setStream(NettyClientStream)，然后drainPendingCalls()调用所有待执行的Runnable，比如说ClientCallImpl.start()方法中的实现逻辑中，在第一次创建的PendingStream时然后调用start()
            //方法时因为判断realStream还没准备好就以Runnable的形式放入到pendingCalls中。
