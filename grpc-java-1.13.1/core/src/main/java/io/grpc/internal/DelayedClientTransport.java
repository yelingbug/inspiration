...
final class DelayedClientTransport implements ManagedClientTransport {
  ...
  @Override
  public final Runnable start(final Listener listener) {//这个方法是在ManagedChannelImpl构造函数中被直接调用的，这里的listener是ManagedChannelImpl.delayedTransportListener，
    //而当reportTransportInUse被真正执行的时候，会调用ManagedChannelImpl.delayedTransportListener.transportInUse(true)，这个方法里又会调用
    //inUseStateAggregator.updateObjectInUse(delayedTransport, inUse=true)，inUseStateAggregator.updateObjectInUse()方法里第一次的时候会调用handleInUse()，再看看
    //inUseStateAggregator的实现里调用了exitIdleMode()方法，这个方法比较关键，因为在第一次ClientCalls.start()的时候真正的网络流还没有建立，而是代替为之相关的
    //DelayedClientTransport及其的PendingStream(extends DelayedStream)，都是放在队列里，等到真正的网络流建立的时候才会发生，这样就可以解释这里的比如
    //reportTransportInUse实际上是个Runnable被增加到调度队列里，被放到后面去执行，那么我们接着看看后面是怎么被触发的？(其实是在第一次调用的时候返回delayedTransport之前
    //channelExecutor中调用了exitIdleMode()并且立即drain()了。)
    ...
    try {
      ...
      synchronized (lock) {
        if (shutdownStatus == null) {
          if (lastPicker == null) {
            return createPendingStream(args);//第一次走到这里是PendingStream
          }
          picker = lastPicker;
  ...
  private PendingStream createPendingStream(PickSubchannelArgs args) {
    ...
    if (getPendingStreamsCount() == 1) {//这个方法的调用者newStream被synchronized了，所以第一个执行到这里的线程肯定为1
      channelExecutor.executeLater(reportTransportInUse);//我们看看reportTransportInUse这个是在什么地方赋值的？是在ManagedChannelImpl的构造函数里new了DelayedClientTransport
      //时，紧接着调用start(listener)方法，reportTransportInUse被赋值了，看这个类里的start(listener)方法。
       //我们再看看start(listener)方法里reportTransportInUse被赋成什么值了？
    }
