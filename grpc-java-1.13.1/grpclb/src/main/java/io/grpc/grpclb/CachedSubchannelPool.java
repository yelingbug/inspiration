...
final class CachedSubchannelPool implements SubchannelPool {
  ...
  public Subchannel takeOrCreateSubchannel(
    ...
    if (entry == null) {
      subchannel = helper.createSubchannel(eag, defaultAttributes);//没有创建一个
    } else {
      subchannel = entry.subchannel;//已经有了，取消10s后关闭subchannel的定时任务，并返回这个subchannel
  ...
  @Override
  public void returnSubchannel(Subchannel subchannel) {//subchannel返回到池中保留10s的时间，时间一到自动关闭，如果没到10s会被重用
