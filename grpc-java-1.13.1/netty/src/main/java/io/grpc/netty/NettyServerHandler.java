...
class NettyServerHandler extends AbstractNettyHandler {
  ...
  private NettyServerHandler(
    ...
    decoder().frameListener(new FrameListener());//这里赋值DefaultHttp2ConnectionDecoder中的listener
    /**
     * 在FrameListener中的onDataRead方法是被Netty库中的{@link Http2InboundFrameLogger#readFrame(ChannelHandlerContext, ByteBuf, Http2FrameListener)}调用的,而
     * {@link Http2InboundFrameLogger#readFrame(ChannelHandlerContext, ByteBuf, Http2FrameListener)}又是被{@link DefaultHttp2FrameReader}中的readDataFrame()方法
     * 调用的,继续往上一层是被{@link DefaultHttp2FrameReader#processPayloadState(ChannelHandlerContext, ByteBuf, Http2FrameListener)}方法的,继续上一层是被
     * {@link DefaultHttp2FrameReader#readFrame(ChannelHandlerContext, ByteBuf, Http2FrameListener)} 调用的,我们在{@link DefaultHttp2FrameReader#processPayloadState(ChannelHandlerContext, ByteBuf, Http2FrameListener)}方法
     * 中看到流结束是通过flags.endOfStream()来判断的,flags这个属性是在{@link DefaultHttp2FrameReader#processHeaderState(ByteBuf)} 方法中通过读取HTTP/2的头数据中拿到的,
     * 根据HTTP/2的规范这个flags中包含流结束的标识.
     */
  }
  ...
  private void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers)
    ...
    /**
     * 走到这里,HTTP/2请求的HEADER被解析,流ID(streamId)已经有了,请求header数据包也被解析成Http2Headers.
     */
    ...
    try {
      ...
      // method.
      Http2Stream http2Stream = requireHttp2Stream(streamId);//这里的stream就是HTTP/2中的对应的流定义封装,在前面解析的时候已经被放入到DefaultHttp2Connection连接,
      //这个连接是底层对应逻辑意义上的连接
      ...
      NettyServerStream stream = new NettyServerStream(
          ...
          authority,
          statsTraceCtx,
          transportTracer);//创建基于Netty的处理服务流
      transportListener.streamCreated(stream, method, metadata);//流已经创建了调用ServerTransportListenerImpl.streamCreated()
  ...
  private void onDataRead(int streamId, ByteBuf data, int padding, boolean endOfStream)
    ...
    try {
      ...
      stream.inboundDataReceived(data, endOfStream);//这里的endOfStream标识是从哪里的看的,参照上面280行的注释
    } catch (Throwable e) {
